...
{
    public class DamageX3ViewModel : SurfaceViewModel<ICircleSurface>
    {
        ...
        public DamageX3ViewModel(ICircleSurface? damage)
        {
            Damage = damage ?? new DamageX3();
        }
        ...
        public override void Draw(IGuidanceInfo guidance, Graphics graphics)
        {
            ...
            using Brush brush = new SolidBrush(Adapt(Color.OrangeRed));

            Single radius = Damage.Radius;
            graphics.FillCircle(brush, Damage.Center.X - radius, Damage.Center.Y - radius, 2 * radius);
            graphics.DrawString("X3", font, Brushes.Azure, Damage.Center, ContentAlignment.MiddleCenter);
        }
    }
}